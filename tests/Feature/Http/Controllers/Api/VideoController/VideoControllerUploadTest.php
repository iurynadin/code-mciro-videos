...
class VideoControllerUploadTest extends BaseVideoControllerTestCase
{
    ...
    use TestValidations, TestUploads, DatabaseMigrations;
    ...
    public function testInvalidationTrailesField()
    {
        $this->assertInvalidationFile(
            'trailer_file',
            'mp4',
    ...
    public function testUpdateWithFiles()
    {
        ...
        $response->assertStatus(200);
        $this->assertFileOnPersist(
            $response, 
            Arr::except($files, ['thumb_file', 'video_file','banner_file']) + $newFiles
        );
    ...
    protected function assertFileOnPersist(TestResponse $response, $files)
    {
        $id = $response->json('id');
        $video = Video::find($id);
